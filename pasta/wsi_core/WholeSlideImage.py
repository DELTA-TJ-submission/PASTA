...
import h5py
import math
from pasta.wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
import itertools
from pasta.wsi_core.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        # Robustly infer mpp; some slides lack openslide.mpp-x/y. Fall back to
        # mpp-y, then objective power heuristic, then a safe default (0.5 µm/px).
        ...
        if self.mpp is None:
            try:
                objective = float(self.wsi.properties.get('openslide.objective-power', 0))
            except Exception:
                objective = 0
            if objective > 0:
                # Approximate: 20x -> ~0.5µm, 40x -> ~0.25µm
                self.mpp = 10.0 / objective
            else:
                self.mpp = None
            warnings.warn(
                f"Missing mpp metadata for slide '{path}', using fallback mpp={self.mpp}"
            )
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        white_black=True, white_thresh=15, black_thresh=30, contour_fn='four_pt', use_padding=True):
