...
from pasta.dataset import H5TileDataset_infer
from pasta.model_utils import post_collate_fn, get_img_transforms, load_model_weights
...
def weight_map_make(start_x, start_y, H, W, weight_matrix, coords, downsample_size=1):
    '''Generate the whole map of weights
    Args:
        start_x: x coordinate of the start of the image
        start_y: y coordinate of the start of the image
        H: height of the image
        W: width of the image
        weight_matrix: the weight matrix
        coords: the coordinates of the patches
        blank: the blank space around the image(default: 3000)
        downsample_size: the size of the downsample
    '''
    op = 3000
    W = W+op
    H = H+op
    patch_size_pxl = weight_matrix.shape[0]
    
    # Work in downsampled space if requested
    if downsample_size > 1:
        H = H // downsample_size
        W = W // downsample_size
        patch_size_pxl = patch_size_pxl // downsample_size
        weight_matrix = weight_matrix_make(patch_size_pxl//2, patch_size_pxl)
        if not torch.is_tensor(weight_matrix):
            weight_matrix = torch.tensor(weight_matrix, dtype=torch.float16)
        op = op // downsample_size  # Scale op for downsampled space
    
    if weight_matrix.shape[0]%2==0:
        r1 = patch_size_pxl//2; r2 = patch_size_pxl//2
    else:
        r1 = patch_size_pxl//2; r2 = patch_size_pxl//2+1
        
    weight_map = torch.zeros((1, H, W), dtype=torch.float16)
    
    for i in tqdm(range(coords.shape[0])):
        y,x = coords[i]
        if downsample_size > 1:
            x = (x-start_x) // downsample_size + op//2
            y = (y-start_y) // downsample_size + op//2
        else:
            x = x-start_x+op//2
            y = y-start_y+op//2
        x,y = int(x), int(y)
        weight_map[0, x-r1:x+r2, y-r1:y+r2] += weight_matrix
    weight_map[weight_map == 0] = 1 
    weight_map = weight_map[:, (op//2):, (op//2):]
    return weight_map


def weight_matrix_make(overlap, patch_size, weight_type='linear', sigma_ratio=0.25):
    ''' Generate weight matrix for blending overlapping patches
    Args:
        overlap: overlap region size
        patch_size: size of the patch
        weight_type: 'linear', 'gaussian', 'cosine', or 'sigmoid'
        sigma_ratio: ratio of sigma to overlap for gaussian (default: 0.25)
    '''
    weight_matrix = np.ones((patch_size, patch_size))
    
    # Generate 1D weight profile based on weight_type
    if weight_type == 'linear':
        weight_1d = np.linspace(0, 1, overlap)
    elif weight_type == 'gaussian':
        sigma = overlap * sigma_ratio
        x = np.arange(overlap)
        weight_1d = np.exp(-0.5 * ((x - overlap) / sigma) ** 2)
    elif weight_type == 'cosine':
        x = np.linspace(0, np.pi/2, overlap)
        weight_1d = np.sin(x)
    elif weight_type == 'sigmoid':
        x = np.linspace(-6, 6, overlap)
        weight_1d = 1 / (1 + np.exp(-x))
    else:
        raise ValueError(f"Unknown weight_type: {weight_type}. Use 'linear', 'gaussian', 'cosine', or 'sigmoid'.")

    # Apply weight on four sides
    for i in range(overlap):
        weight_matrix[i, :] *= weight_1d[i]
        weight_matrix[-i-1, :] *= weight_1d[i]
        weight_matrix[:, i] *= weight_1d[i]
        weight_matrix[:, -i-1] *= weight_1d[i]

    # Apply weight on four corners
    for i in range(overlap):
        for j in range(overlap):
            min_weight = min(weight_1d[i], weight_1d[j])
            weight_matrix[i, j] = min_weight
            weight_matrix[i, -j-1] = min_weight
            weight_matrix[-i-1, j] = min_weight
            weight_matrix[-i-1, -j-1] = min_weight
...
def predict_pixel_level(tile_h5_path, model, img_transforms, wsi_path, edge_info_path, pathway_info, patch_size, output_path='/workspace/results/predictions', save_h5ad=False, save_tiffs=False, downsample_size=10, figsize=10, device='cuda:0', file_type='.svs', draw_images=True, cmap='turbo', use_mask=False, blank=3000): 
...
def predict_spot_level(tile_h5_path, model, img_transforms, pathway_names, output_path='/workspace/results/predictions_h5ad', device='cuda:0', wsi_path=None, file_type='.svs', use_mask=False, blank=3000, downsample_size=10, filter_background=True):
